...
@Slf4j
public class ContactWithoutCompanyHateoasRestClient implements HateoasLinkRestClient<ListHateoasEntity<RequestContactWithoutCompanyDTO>> {
    private final RestClient restClient;
    @Override
    public Optional<ListHateoasEntity<RequestContactWithoutCompanyDTO>> request(Link value) {
        log.info("request to link: {}", value);
        return Optional.ofNullable(restClient.get()
                .uri(value.href())
                .retrieve()
                .body(ListResponseContactsWithoutContactDTO.class))
                .map(ListResponseContactsWithoutContactDTO::getEntity);
    }
}
